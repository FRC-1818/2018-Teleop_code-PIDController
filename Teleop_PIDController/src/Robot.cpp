...
public:
	Robot() {
		...
		arm_Controller1.SetInputRange(0.0, 5.0);
		arm_Controller1.SetOutputRange(-1.0, 1.0);
		arm_Controller1.SetContinuous(true);
		arm_Controller1.Disable();

		arm_Controller2.SetInputRange(0.0, 5.0);
		arm_Controller2.SetOutputRange(-1.0, 1.0);
		arm_Controller2.SetContinuous(true);
		arm_Controller2.Disable();
	}
	...
	void OperatorControl() override {
		...
		while (IsOperatorControl() && IsEnabled()) {
			...
			SmartDashboard::PutNumber("ArmPosition", ArmPosition.GetVoltage());
			...
			arm_Controller1.Enable();
			arm_Controller2.Enable();
			arm_Controller1.SetSetpoint(1.40);
			arm_Controller2.SetSetpoint(1.40);
			...
			/*-----Arm Operation-----*/
			if (m_DownFront == 1) {
				arm_Controller1.SetSetpoint(3.00);
				arm_Controller2.SetSetpoint(3.00);
			} else if (m_UpFront == 1) {
				arm_Controller1.SetSetpoint(2.10);
				arm_Controller2.SetSetpoint(2.10);
			} else if (m_Vertical == 1) {
				arm_Controller1.SetSetpoint(1.40);
				arm_Controller2.SetSetpoint(1.40);
			} else if (m_Behind == 1) {
				arm_Controller1.SetSetpoint(0.20);
				arm_Controller2.SetSetpoint(0.20);
			}
		}
	}
...
private:
	...
	double kd = 0.0;
	double ki = 0.0;
	frc::PIDController arm_Controller1{kp, ki, kd, &ArmPosition, &m_leftlift};
	frc::PIDController arm_Controller2{kp, ki, kd, &ArmPosition, &m_rightlift};
